...
class DemoDepositController(private val service: DepositApplicationService, private val metrics: CasinoWalletMetrics) {
    private val log = LoggerFactory.getLogger(javaClass)

    @PostMapping("/api/demo/deposits/{depositId}/complete")
    fun complete(@PathVariable depositId: UUID): CallbackResponse {
        val result = metrics.depositCallback(DepositMetricOperation.DEMO_COMPLETION) {
            when (val outcome = service.completeDemo(depositId)) {
